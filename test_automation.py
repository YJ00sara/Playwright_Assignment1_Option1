...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "Assignment 1 - Test cases-IT23861336.xlsx"),
]
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if input_by_ph.count() > 0 and output_by_ph.count() > 0 and input_by_ph.is_visible() and output_by_ph.is_visible():
                action = page.get_by_role("button", name=re.compile(r"^(Translate|Transliterate)$", re.IGNORECASE)).first
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if action_locator:
                    try:
                        action_locator.click(timeout=3000)
                    except Exception:
                        pass
